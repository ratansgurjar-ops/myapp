...
// In development (with HMR) or serverless environments the module may be
// re-evaluated multiple times which can create multiple pools. Store the
// pool on `global.__mysqlPool` so it is reused across reloads.
...
async function getPool() {
  ...
  try {
    ...
    // persist to global so HMR/requires reuse same pool in dev
  ...
  // Typing tutor: shared manual content and user exercises
  try {
    await pool.query(`
    ...
    console.warn('Could not ensure typing_shared table:', e.message || e);
  }
  ...
  try {
    ...
    console.warn('Could not ensure typing_user_exercises table:', e.message || e);
  }
  ...
  try {
    ...
    console.warn('Could not ensure typing_users table:', e.message || e);
  }
