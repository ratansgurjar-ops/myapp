...
export default function Advertisements({ items = [], total = 0, page = 1 }) {
  return (
      ...
      <div style={{ marginTop: 12 }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          ...
          <div style={{ marginLeft: 'auto' }}>
            <a href="/news?type=all">All</a> | <a href="/news?type=news">News</a> | <a href="/advertisements">Ads</a>
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12 }}>
          {items.map(it => {
            const href = (it.link && it.link.length) ? it.link : `/advertisements/${it.slug}`;
            return (
...
export async function getServerSideProps(context) {
  ...
  const data = await News.findNews({ page, limit, type: 'ad', includeInactive: false });
  ...
  return { props: { items: data.items || [], total: data.total || 0, page: page } };
}
